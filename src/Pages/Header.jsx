...
function Header({ darkMode, setDarkMode }) {
  ...
  return (
    <div
      className={`h-32 ${
        isScrolled ? "bg-black/60 backdrop-blur-lg text-white" : "bg-none"
      } p-2 sticky top-0 z-50 overflow-hidden`}
    >
      <div className="relative flex justify-end items-center mb-1 mt-1">
        <div
          className={`absolute inset-0 transition-all duration-500 ease-in-out ${
            shouldDisplay ? "opacity-100 scale-100" : "opacity-0 scale-90"
          }`}
        >
          <Greet />
        </div>
        <div
          className={`absolute inset-0 transition-all duration-500 ease-in-out ${
            shouldDisplay ? "opacity-0 scale-90" : "opacity-100 scale-100"
          }`}
        >
          <Logo />
        </div>
